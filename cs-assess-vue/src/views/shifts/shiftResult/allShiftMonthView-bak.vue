    ...
    <el-dialog :title="dayShiftTitle" :visible.sync="dialogDayShiftVisible" width="1175px" class="day-shift-dialog" :close-on-click-modal="false">
      <el-row class="dialog-inner">
        <el-form :model="addForm" ref="addForm" :rules="rules">
          <el-row class="day-shift-dialog_left" data-before="修改班次">
          ...
          getDayShiftView4Edit(params).then(res => {
            res.data.forEach((item, index) => {
              ...
              if (item.userIdList && item.userIdList.length > 0) {
                // 将重复人员过滤
                item.userIdList = item.userIdList.filter(function(element, index, self) {
              ...
              if (item.userInfoList && item.userInfoList.length > 0) {
                ...
                const obj = {}
                let tmpUserList = []
                tmpUserList = item.userInfoList.reduce(function(item1, next) {
                  obj[next.id] ? '' : obj[next.id] = true && item1.push(next)
                  return item1
                }, [])

                tmpUserList.forEach((user, index1) => {
                  item.userList.push({
                    key: user.id,
                    label: user.trueName
                  })
