<template>
  <el-row class="allShiftMonthView">
    ...
    <!--班务月视图 end-->
    <!-- 规则排班弹窗 start -->
    <el-dialog title="规则排班" :visible.sync="dialogRegularVisible" width="800px" class="rule-dialog" :close-on-click-modal="false">
      <el-form :model="regularForm" ref="regularForm" :rules="rules">
        <el-form-item label="组织结构" prop="csaOrgId" :label-width="formLabelWidth">
          <org-tree ref="orgTree" v-model="regularForm.csaOrgId" @getUsersUnlimited="getUsersUnlimited(regularForm)"></org-tree>
        </el-form-item>
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="hideRegularDialog('regularForm')">取 消</el-button>
    ...
    <!-- 规则排班弹窗 end -->
    <!-- 普通排班弹窗 start -->
    <el-dialog title="普通排班" :visible.sync="dialogOrdinaryVisible"  width="1175px" class="rule-dialog" :close-on-click-modal="false">
      <el-form :model="ordinaryForm" ref="ordinaryForm" :rules="rules">
        <el-row class="rd-left">
          ...
          <el-form-item label="组织结构" prop="csaOrgId" :label-width="formLabelWidth">
            <org-tree ref="orgTree" v-model="ordinaryForm.csaOrgId" @getUsersUnlimited="getUsersUnlimited(ordinaryForm)"></org-tree>
          </el-form-item>
          <el-form-item label="选择客服" prop="userIdList" :label-width="formLabelWidth">
            <el-transfer
              filterable
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="hideOrdinaryDialog('ordinaryForm')">取 消</el-button>
        <el-button type="primary" @click="schedulingWithOrdinary('ordinaryForm')">提 交</el-button>
      </div>
    </el-dialog>
    <!-- 普通排班弹窗 end -->
    <!-- 编辑日排班 start -->
    <el-dialog :title="dayShiftTitle" :visible.sync="dialogDayShiftVisible" width="1175px" class="day-shift-dialog" :close-on-click-modal="false">
    ...
    methods: {
        ...
        this.$refs[form].validate((valid) => {
          if (valid) {
            ...
            for (var i = 0; i < _this.allDaysOfCurrentMonth.length; i++) {
              ...
              _shiftId = _shiftId === '' ? '' : _shiftId + 1 // 已选择的班次id从1开始
      ...
      showDayShiftForm(date) {
        ...
        Promise.all([p1, p2]).then((dataArray) => {
          _this.loading = false
          _this.allUserLists = dataArray[0].data // p1的数据(所有客服)
          // p2的数据(班次信息)
          _this.oldShiftInfo = dataArray[1].data
          _this.oldShiftInfo.forEach(item => {
            const hash = {}
            item.userList = _this.allUserLists.reduce((item, next) => {
       ...
       */
      getUsersUnlimited(form, orgTree, shiftInfo) {
        if (form) {
          if (this.$refs.orgTree) {
            const aOrgTree = this.$refs.orgTree.selectedOptions
            form.csaOrgId = ''
          ...
          if (shiftInfo) {
            shiftInfo.forEach((item, index) => {
              if (this.$refs[orgTree + index]) {
                const aOrgTree = this.$refs[orgTree + index][0].selectedOptions
                item.csaOrgId = ''
                if (aOrgTree && aOrgTree.length > 0) {
                  item.csaOrgId = aOrgTree[aOrgTree.length - 1]
                }
                if (item.csaOrgId && item.csaOrgId !== '') {
                  return new Promise((resolve, reject) => {
                    getUsersUnlimited({ csaOrgId: item.csaOrgId, status: 0 }).then(res => {
                      res.data.forEach((item1, index1) => {
                        if (this.allUserLists[index1].key === item1.key) {
                      ...
                      this.$nextTick(() => {
                        this.$refs.userList.data = res.data
                        this.$set(this.$refs, 'userList.data', res.data)
                        console.log(res.data)
                      })
                      // this.$set(item, 'a', '11')
                      // this.$set(item, 'userList', res.data)
                      resolve(res)
                    }).catch(error => {
                      reject(error)
                    })
      ...
      // 关闭规则弹窗
      hideRegularDialog(form) {
        this.dialogRegularVisible = false
        this.$refs[form].resetFields()
      },

      // 关闭普通弹窗
      hideOrdinaryDialog(form) {
