...
<script>
  ...
  import { getMonthShiftView, getDayShiftView4Edit, insertShiftResultBatch, deleteShiftResultBatch } from '@/api/shifts/shift-result-api'
  ...
  export default {
    ...
    methods: {
      ...
      resetQueryForm(form) {
        ...
        this.queryForm.shiftDate = new Date()
            ...
            _this.$confirm('排班人数' + params.userIdList.length + '人，排班天数' + aShiftDays.length + '天, 是否继续?', '提示', {
              ...
              return new Promise((resolve, reject) => {
                schedulingWithOrdinary(params).then(res => {
                  ...
                  _this.$refs.orgTreeOrdinary.selectedOptions = []
      ...
      showDayShiftForm(date) {
        ...
        Promise.all([p1, p2]).then((dataArray) => {
          ...
          dataArray[1].data.forEach(item => {
            ...
            item.userIdList = item.userIdList.map(String)
            item.selectedUserLists = item.userIdList
          })
      ...
      getUsersUnlimited(form, orgTree, shiftInfo) {
        if (form) {
          ...
          if (shiftInfo) {
            ...
            //   item.csaOrgId = ''
            //   const userListFromShiftInfo = item.userList
            //   const userIdListFromShiftInfo = item.userIdList
      ...
      submitAddForm(form) {
        const _this = this
        let shiftResultList = []
        let removedUserLists = [] // 移走的人员的id数组
        let removeUserListParams = [] // 移走人员的参数数组，存放日期和人员id
        // 修改班次信息
        this.oldShiftInfo.forEach(item => {
          if (item.userIdList && item.userIdList.length > 0) {
            ...
            removedUserLists = item.selectedUserLists.filter(cur => !item.userIdList.includes(cur))
            // 收集移走人员的id和班次日期
            if (removedUserLists.length > 0) {
              removedUserLists.forEach(itemRemoved => {
                removeUserListParams.push({
                  csaUserId: parseInt(itemRemoved),
                  shiftDate: _this.addForm.shiftDate
                })
              })
            }
        ...
        new Promise((resolve, reject) => {
          deleteShiftResultBatch(removeUserListParams).then(res => {
            removeUserListParams = [] // 提交成功之后将参数数组清空
            resolve(res)
          }).catch(error => {
            reject(error)
          })
      ...
      uploadShiftResultExcel(e) {
        ...
        const _this = this
        let timer = null
        this.excelForm.shiftDate = formatDateByTimestamp(new Date(this.excelForm.shiftDate), 'yyyy-MM')
        // 文件对象
