...
service.io.connect = function(option) {
  if (!service.validate) {
    MessageBox.alert('当前浏览器不支持来电通知，请使用新版的谷歌浏览器', '提示', {
      type: 'error',
  ...
  const topics = {
    ...
    'message': function(data) {
      if (data) {
        socket.send('H:1\nN:publishMessage\nS:' + service.sequence.getSequenceByName(data['publishMessage']) + '\n\n' + JSON.stringify(data))
      }
    },
    'postmessage': function(data) {
      setTimeout(function() {
        socket.send('H:1\nN:' + data['N'] + '\nS:' + service.sequence.getSequenceByName(data['N']) + '\n\n' + JSON.stringify(data['args']))
      }, 0)
    }
  }
  ...
  emitter.flush = function(funcName, argsObj) {
    ...
    if (!service.DataStream[funcName]) {
      ...
      service.DataStream[funcName]['timer'] = setTimeout(function() {
        emitter(funcName, service.DataStream[funcName]['arg'].hasOwnProperty('arg') ? service.DataStream[funcName]['arg']['arg'] : service.DataStream[funcName]['arg'])
      }, 1000)
    }
    if (Object.prototype.toString.call(argsObj) === '[object Array]') {
      service.DataStream[funcName]['arg'] = { 'arg': argsObj }
    } else {
      service.DataStream[funcName]['arg'] = JSON.parse('{"' + argsObj['personId'] + '":"' + argsObj['code'] + '"}')
    }
    return { result: true, content: 'success' }
  }
  ...
  socket.onmessage = function(evt) {
    ...
    if (con.length === 0) {
      ...
      const run = emitter(con['N'], JSON.parse(con['args'] === '' ? '{"C":"2000"}' : con['args']))
      if (!run.result) {
  ...
  return {
    ...
    emit: function(content, callback) {
      if (socket && service.io.isopen) {
        ...
        try {
          ...
          throw new Error('Send Error,please check socket setting,errorMessage:' + err.message)
        }
      }
    },
    close: function() {
    ...
    sendSingal: function(msg) {
      if (socket && socket.readyState === socket.OPEN) {
        ...
        MessageBox.alert('你已与服务器断开连接，请检查网络或联系管理员', '提示', {
          type: 'error',
          confirmButtonText: '确定',
          callback: action => {
